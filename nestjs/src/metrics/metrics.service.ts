...
export class MetricsService {
  ...
  async getDashboardWithWidgets(dashboardId: string): Promise<any | null> {
    this.logger.log(`Fetching dashboard with ID: ${dashboardId}`);
    try {
      const dashboard =
        await this.supabaseService.getDashboardAndWidgetsById(dashboardId);
      if (!dashboard) {
        this.logger.warn(`Dashboard with ID "${dashboardId}" not found.`);
        return null;
      }
      this.logger.log(
        `Successfully fetched dashboard "${dashboard.name}" with ID: ${dashboardId}`,
      );
      return { success: true, data: dashboard };
    } catch (error) {
      this.logger.error(
        `Failed to fetch dashboard with ID "${dashboardId}": ${error.message}`,
      );
      return { success: false, data: null, error: error.message };
    }
  }
  async getAllDashboards(): Promise<any[]> {
    this.logger.log('Fetching all dashboards.');
    try {
      const data = await this.supabaseService.getAllDashboardsMinimal();
      this.logger.log(`Successfully fetched ${data.length} dashboards.`);
      return data;
    } catch (error) {
      this.logger.error(`Failed to fetch all dashboards: ${error.message}`);
      return [];
    }
  }
